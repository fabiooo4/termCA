...
pub fn ant_turn(app: &mut App) {
    for (state, rule) in app.ant_sim.states.iter().zip(app.ant_sim.rules.iter()) {
        if app.ant_sim.ant_grid.cells[app.ant_sim.ant.y as usize][app.ant_sim.ant.x as usize] == *state {
            match rule {
                Direction::Left => {
...
pub fn ant_flip(app: &mut App) {
    ...
    while let Some(state) = states.next() {
        if app.ant_sim.ant_grid.cells[app.ant_sim.ant.y as usize][app.ant_sim.ant.x as usize] == *state {
            app.ant_sim.ant_grid.cells[app.ant_sim.ant.y as usize][app.ant_sim.ant.x as usize] =
                states.next().unwrap().clone();
            break;
        }
    }
}
